        ...
        createDeployment: {
            ...
            provideCode: function(deploymentInfo) {
                ...
                prompt.get(['code'], function(err, result) {
                    ...
                    actions.createDeployment.getConditions(deploymentInfo, activeSpace, bearerKey);
                });
            },
            /* Method lists out all active conditions in the space for user to select from */

            getConditions: function(deploymenInfo, activeSpace, bearerKey) {
                ...
                request({
                    ...
                    if (response.statusCode !== 200) {
                        ...
                        for (var i = 0; i < conditions.length; i++) {
                            for (var j = 0; j < conditions[i].deployments.length; j++) {
                                if (conditions[i].deployments[j].spaceId == activeSpace.id) {
                                    ...
                                    cNameArray.push(conditions[i].name);
                                    cIdArray.push(conditions[i].id);
                        ...
                        for (var i = 0; i<cNameArray.length; i++){
                            console.log(cIdArray[i] + ': ' + cNameArray[i]);
                        }
